...
esp_err_t bsp_extra_i2s_read(void *audio_buffer, size_t len, size_t *bytes_read, uint32_t timeout_ms)
{
    if (!record_dev_handle) {
        ESP_LOGE(TAG, "I2S read failed: record handle is NULL");
        *bytes_read = 0;
...
esp_err_t bsp_extra_codec_dev_stop(void)
{
    ...
    // Close and delete play device to free memory
    if (play_dev_handle) {
        ret = esp_codec_dev_close(play_dev_handle);
        // Delete the device to free allocated memory
        esp_codec_dev_delete(play_dev_handle);
        play_dev_handle = NULL;
    }

    // Close and delete record device to free memory
    if (record_dev_handle) {
        ret |= esp_codec_dev_close(record_dev_handle);
        // Delete the device to free allocated memory
    ...
    // Reset the init flag so codec can be reinitialized next time
    _is_audio_init = false;
    ESP_LOGI(TAG, "Codec fully stopped and memory freed");
...
esp_err_t bsp_extra_codec_init()
{
    ...
    esp_err_t ret = bsp_extra_codec_set_fs(CODEC_DEFAULT_SAMPLE_RATE, CODEC_DEFAULT_BIT_WIDTH, CODEC_DEFAULT_CHANNEL);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to set codec sample format: %s", esp_err_to_name(ret));
        return ret;
    }

    _is_audio_init = true;
    ESP_LOGI(TAG, "Audio codec fully initialized");
